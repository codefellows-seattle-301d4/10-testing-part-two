...
var tooHungryDay = 0;
var averageMeals = 0;
var totalMealsEaten = 0;

(run = function(){
  for(var i = 0; i < mealsPerDay.length; i++){
    totalMealsEaten += (mealsPerDay[i]);
    averageMeals = (totalMealsEaten/(i + 1)).toFixed(1);
    tooHungryDay = (i + 1);
    console.log('On day ' + (i + 1) + ', the lion has been given an average of ' + averageMeals + ' meals.');
    if(averageMeals < 4){
      break;
    }
  }
...
expect(
  tooHungryDay > 0 && tooHungryDay < 10,
  'tooHungryDay should be a number less than 10 but is ' + tooHungryDay,
  'The lion got too hungry before the days in the array ran out, stupid caretaker.');
